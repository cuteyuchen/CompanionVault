...
with Path(args.output).open("w",encoding="utf-8") as f:
    for m in messages: f.write(json.dumps({"sender":pseudonym(m.sender),"text":redact_text(m.text,names) if args.public else m.text,"timestamp":m.timestamp},ensure_ascii=False)+"\n")
